...
Additional code for different output formats located in `/output`.


## prepare.py

Generally the user provides an input file or dictionary with the corresponding parameters for the processing. 
From this many additional options are derived.
This happens in the `Preparation` class.
The following functionality is handled by the `Preparation` class.
