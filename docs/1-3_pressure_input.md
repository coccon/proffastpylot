...
An example pressure type file is provided in `example/log_type_pressure.yml`, describing the KIT-style data format. The path to the pressure files recorded by the KIT datalogger need to be given as `pressure_path`. To adapt this file two your own file format, the options are explained in the following.
