...
def prepare_pressure_df(
        pressurePath, pArgs, dayList):
    ...
    if frequency == "subdaily":
        p_dict = read_subdaily_files(pressurePath, pArgs, dayList)
        ...
        p_dict = read_yearly_files(pressurePath, pArgs, dayList)
    return p_dict


def read_subdaily_files(pressurePath, pArgs, dayList):
    ...
    for day in dayList:
        ...
        for file in dataloggerFileList:
            temp = pd.read_csv(
                file,
                **(pArgs["dataframe_parameters"]["csv_kwargs"]))
            daily_df = pd.concat([temp, daily_df])
        daily_df = _parse_datetime_col(daily_df, pArgs, day)
        pressure_dict[day] = _to_pressure_list(daily_df, pArgs)
    return pressure_dict


def read_yearly_files(pressurePath, pArgs, dayList):
    """read yearly files and return a dict containing the pressure
    for each day in dayList
    """
    first_year = dayList[0].year
    last_year = dayList[-1].year
    ...
    for year in years:
        filename = _get_filename(
            ...
            params=pArgs["filename_parameters"])
        fileList = glob.glob(
            os.path.join(pressurePath, filename))
        if len(fileList) > 1:
    ...
    pdtc = "parsed_datecol"
    df = _parse_datetime_col(df, pArgs)
    p_dict = {}
    ...
    for day in dayList:
        ...
        p_dict[day] = _to_pressure_list(dailyDf, pArgs)
    return p_dict
...
def generate_pt_intraday(p_list, template_path):
    ...
    """
    p_lines = _create_p_lines(p_list)
...
def _parse_datetime_col(df, pArgs, date=None):
    """parse the dataframe for a suitable datetime. Add the column
    'parsed_datecol' to the dataframe
    """
    df_args = pArgs["dataframe_parameters"]
    ...
    if datetime_key == "":
        ...
        if date_key == "":
            # no date key avaliable as well. Do only take the time from file.
...
def _to_pressure_list(df, pArgs):
    ...
    # Filter values which are too large or too small. Replace or remove them.
    maxVal = float(pArgs["data_parameters"]["max_pressure"])
    minVal = float(pArgs["data_parameters"]["min_pressure"])
    replace_val = 0
    if pArgs["data_parameters"]["default_value"] == "skip":
        replace_val = np.nan
    else:
        replace_val = float(pArgs["data_parameters"]["default_value"])
