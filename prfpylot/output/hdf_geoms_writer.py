...
class GeomsGenWriter(GeomsGenHelper):
    def __init__(self, geomsgen_inputfile, geoms_out_path=None):
        ...
        super(GeomsGenWriter, self).__init__(geomsgen_inputfile, geoms_out_path)
    ...
    def generate_GEOMS_at(self, day):
        ...
        if (vmr is None) or (ptf is None) or (sen is None):
            self.logger.error("HDF file generation stopped while reading VMR, pT, or colsens file!")
            self.MyHDF5.close()
            return
        ...
        self.logger.info(
            f"The HDF5geoms file for {day.strftime('%Y-%m-%d')} was written to {geoms_file_name}"
        )
    ...
    def get_colsens_sza(self, day):
        ...
        path = os.path.join(self.prf_res_path, "raw_output_proffast", colsens_filename)
    ...
    def _get_ils_form_preprocess_inp(self, day):
        ...
        search_path = os.path.join(self.prf_res_path, "input_files", f"preprocess*{yymmdd_str}.inp")
    ...
    def write_metadata(self, day, df):
        ...
        if self.DATA_DESCRIPTION is None:
            self.MyHDF5.attrs["DATA_DESCRIPTION"] = np.bytes_(
                f"EM27/SUN ({self.instrument_number}) measurements from {self.DATA_LOCATION}."
            )
        else:
            self.MyHDF5.attrs["DATA_DESCRIPTION"] = np.bytes_(self.DATA_DESCRIPTION)
    ...
    def generate_geoms_files(self):
        ...
        self.logger.info(
            f"Generating HDF5geoms files between {self.geoms_start_date} and {self.geoms_end_date}."
        )
        ...
        if self.ils_not_in_file_warning is True:
            self.logger.warning(
                "The ILS could not be determined and no values are written to the geoms metadata."
            )
