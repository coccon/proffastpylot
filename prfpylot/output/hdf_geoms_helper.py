...
class GeomsGenHelper:
    ...
    def __init__(self, geomsgen_inputfile, geoms_out_path=None):
        ...
        prf_input_filepath = os.path.join(self.prf_res_path, self.input_file_name)
    ...
    def _get_correction_factors(self):
        ...
        df = pd.read_csv(self.calibration_params_list, skipinitialspace=True)
    ...
    def apply_quality_checks(self, df):
        ...
        for index, row in df.iterrows():
            ...
            if row[["XCO2", "XCH4", "XH2O", "XCO"]].isin([np.nan, 0.0]).any():
                quality_check_passed = False
                self.logger.debug(
                    f"line {index} removed: ['XCO2', 'XCH4', 'XH2O', 'XCO'] = "
                    f"{row[['XCO2', 'XCH4', 'XH2O', 'XCO']].values}"
                )
    ...
    def get_datetimes(self):
        ...
        colsens_files = glob.glob(os.path.join(inp_path, "*colsens*.dat"))  # invparms file list
