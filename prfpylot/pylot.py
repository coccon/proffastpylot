...
class Pylot(FileMover):
    ...
    def run(self, n_processes=1):
        ...
        if self.geomsgen_inputfile is not None:
            geoms_out_path = os.path.join(self.result_folder, "output_hdf_geoms")
            geoms_writer = GeomsGenWriter(self.geomsgen_inputfile, geoms_out_path=geoms_out_path)
            geoms_writer.generate_geoms_files()

        self.logger.info(f"The results of PROFFAST were written to {self.result_folder}/ .")

    def run_preprocess(self, n_processes=1):
        """Main method to run preprocess."""
        if self.start_with_spectra is True:
            self.logger.info(
                "Running with option: 'start_with_spectra', skipping preprocessing. ...\n"
            )
            return

        self.logger.info(f"Running preprocess with {n_processes} task(s) ...")
        ...
        if n_processes <= 1:
            for inputfile in all_inputfiles:
                tmp_out = self.run_prf_with_inputfile(
                    inputfile, prep_exe, popen_kwargs={"cwd": exec_path}
                )
                output.append(tmp_out)
        else:
            self.logger.debug("...start parallel processing...")
            subs_method = partial(
                self.run_prf_with_inputfile, executable=prep_exe, popen_kwargs={"cwd": exec_path}
            )
    ...
    def run_pcxs(self, n_processes=1):
        ...
        for local_date in temp:
            ...
            if os.path.exists(os.path.join(wrk_fast_path, srchstrg)):
                message = (
                    f"*.abscos.bin file for day {local_date} exists already. Skip calculation.."
                )
                self.logger.info(message)
                output.append([message, "", "No return code", "No call String"])
                continue
            # Generate/find map files
            success = self.prepare_map_file(local_date)
            if not success:
                self.logger.warning(f"Skip day {local_date} since no map file is present")
        ...
        if n_processes <= 1:
            ...
            subs_method = partial(
                self.run_prf_with_inputfile, executable=pcxs_exe, popen_kwargs={"cwd": exec_path}
            )
            pool = multiprocessing.Pool(processes=n_processes)
    ...
    def run_inv(self, n_processes=1):
        ...
        for local_date in temp_list:
            input_files, skipped_spectra = self.generate_invers_input(local_date)
        ...
        if len(p_data_warnings) != 0:
            warn_strg = "Due to missing auxiliary data the following spectra were skipped:"
        ...
        if n_processes <= 1:
            ...
            subs_method = partial(
                self.run_prf_with_inputfile, executable=inv_exe, popen_kwargs={"cwd": exec_path}
            )
            pool = multiprocessing.Pool(processes=n_processes)
            output = pool.map(subs_method, all_inputfiles)
    ...
    def run_prf_with_inputfile(self, prf_inputfile, executable, popen_kwargs={}):
        ...
        outlist = out, err, str(return_val), " ".join([executable, prf_inputfile])
        return outlist
    ...
    def combine_results(self):
        ...
        combined_file = os.path.join(self.result_folder, resultfile)
        ...
        np.savetxt(
            combined_file, df.values, header=header, fmt=format_list, delimiter=", ", comments=""
        )
    ...
    def _get_merged_df(self):
        ...
        search_str = os.path.join(self.raw_output_prf_folder, f"{self.site_name}*-invparms_?.dat")
        invparms_filelist = sorted(glob(search_str))
        if len(invparms_filelist) == 0:
            raise RuntimeError("Retrieval did not produce any *invparms*.dat files")

        df_list = [
            pd.read_csv(file, delimiter=",", skipinitialspace=True) for file in invparms_filelist
        ]
    ...
    def _add_timezones_to(self, df):
        ...
        if 0.0 in df["JulianDate"].values:
        ...
        df["JulianDate"] = df["JulianDate"].replace(0.0, np.nan)
        df.dropna(subset=["JulianDate"], inplace=True)

        df["UTC"] = pd.to_datetime(df["JulianDate"].values, origin="julian", unit="D", utc=True)
    ...
    def _get_executable(self, program):
        ...
        if program == "prep":
            executable = os.path.join(
                self.proffast_path, "preprocess", self.template_types[program]
            )
        else:
            executable = os.path.join(self.proffast_path, self.template_types[program])
