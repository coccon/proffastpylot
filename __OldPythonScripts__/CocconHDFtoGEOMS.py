...
#doi_address = 'https://doi.org/xx.xxxxx/xxxxxxxx' # EM27SUN SN037, KIT, Karlsruhe, Germany
#doi_address = 'https://doi.org/xx.xxxxx/xxxxxxxx' # EM27SUN SN039, FRM4GHG, FMI, Sodankyla, Finland
...
# test file lenght
  ...
  with h5py.File(file_path, 'r') as fid:
    ...
    if (len(fid['DateTime']) < 12): return

#-------------------------------------------------------------------------------------------------#
# function to store GEOMS data (this only calls hdf_store...)

  def GEOMS_store(tem, data, si, m=None, strict=strict, M=None, fill=-900000.0, note=''):
...
# get all information to construct output file name
  ...
  with h5py.File(file_path, 'r') as fid:
    ...
    if fileattrs['DATA_LOCATION'] == 'ARRIVAL': fileattrs['DATA_LOCATION'] = 'ARRIVAL.HEIGHTS'
    if fileattrs['DATA_LOCATION'][0:6] == 'MADRID': fileattrs['DATA_LOCATION'] = 'MADRID'
    if fileattrs['DATA_LOCATION'][0:6] == 'MUNICH': fileattrs['DATA_LOCATION'] = 'MUNICH.TUM'
    ...
    alldatetimes = myDef.GEOMStoDateTime(np.round(fid['DateTime'][...] * 86400.0))

    if iname=='EM27SUN' and isite=='karlsruhe':
        fileattrs['DATA_SOURCE'] += 'KIT.IMK.ASF037';
        fileattrs['PI_NAME'] = 'LastNamePI;FirstNamePI';
      ...
      # fileattrs['PI_AFFILIATION'] = 'Karlsruhe Institute of Technology - Institute of Meteorology and Climate Research - Atmospheric Trace Gases and Remote Sensing;KIT.IMK.ASF';
        ...
        fileattrs['PI_EMAIL'] = 'FirstNamePI.LastNamePI@university.edu';
        fileattrs['DO_NAME'] = 'LastNameDO;FirstNameDO';
      ...
      # fileattrs['DO_AFFILIATION'] = 'Karlsruhe Institute of Technology - Institute of Meteorology and Climate Research - Atmospheric Trace Gases and Remote Sensing;KIT.IMK.ASF';
        ...
        fileattrs['DO_EMAIL'] = 'FirstNameDO.LastNameDO@university.edu';
        fileattrs['DS_NAME'] = 'LastNameDS;FirstNameDS';
      ...
      # fileattrs['DS_AFFILIATION'] = 'Karlsruhe Institute of Technology - Institute of Meteorology and Climate Research - Atmospheric Trace Gases and Remote Sensing;KIT.IMK.ASF';
        ...
        fileattrs['DS_EMAIL'] = 'FirstNameDS.LastNameDS@university.edu'
    elif iname=='EM27SUN' and isite=='sodankyla':
        fileattrs['DATA_SOURCE'] += 'FRM4GHG032';
        fileattrs['PI_NAME'] = 'LastNamePI;FirstNamePI';
        fileattrs['DS_AFFILIATION'] = 'Karlsruhe Institute of Technology;KIT';
      # fileattrs['PI_AFFILIATION'] = 'Karlsruhe Institute of Technology - Institute of Meteorology and Climate Research - Atmospheric Trace Gases and Remote Sensing;KIT.IMK.ASF';
        fileattrs['PI_ADDRESS'] = 'P.O. Box 3640;D-76021 Karlsruhe;GERMANY';
...
# ...
      ...
      GEOMS_store(dict(name='LATITUDE.INSTRUMENT',dtype='REAL',unit='deg',desc='Instrument geolocation (+ for north; - for south)',dep='DATETIME'),fid['Instr_Lat'][...],si='0.0;1.74533E-2;rad')
      GEOMS_store(dict(name='LONGITUDE.INSTRUMENT',dtype='REAL',unit='deg',desc='Instrument geolocation (+ for east; - for west)',dep='DATETIME'),fid['Instr_Lon'][...],si='0.0;1.74533E-2;rad')
      GEOMS_store(dict(name='ALTITUDE.INSTRUMENT',dtype='REAL',unit='km',desc='Instrument geolocation',dep='DATETIME'),fid['Instr_Alt'][...]/1000.0,si='0.0;1.0E3;m')
...
# column mixing ratios, dry air mole fraction error (uncertainty)
    ...
    # error calculation
      ...
      XH2O_val = np.zeros(fid['DateTime'].shape)
      XCO2_val = np.zeros(fid['DateTime'].shape)
      XCH4_val = np.zeros(fid['DateTime'].shape)
      XCO_val  = np.zeros(fid['DateTime'].shape)
      ...
      for i in range(fid['DateTime'].shape[0]-AvgNr+1):
        ...
        XH2O_val[i+ErrNr] = float(fid['XH2O'][i+ErrNr])
        XCO2_val[i+ErrNr] = float(fid['XCO2'][i+ErrNr]) # * float(fid['XCO2_cal'][...])
        XCH4_val[i+ErrNr] = float(fid['XCH4'][i+ErrNr]) # * float(fid['XCH4_cal'][...])
        XCO_val[i+ErrNr]  = float(fid['XCO'][i+ErrNr]) * 1000.0
        ...
        if (float(fid['XH2O'][i+ErrNr]) == -900000.0): XH2O_err[i+ErrNr] = -900000.0; XH2O_val[i+ErrNr] = -900000.0;
        if (float(fid['XCO2'][i+ErrNr]) == -900000.0): XCO2_err[i+ErrNr] = -900000.0; XCO2_val[i+ErrNr] = -900000.0;
        if (float(fid['XCH4'][i+ErrNr]) == -900000.0): XCH4_err[i+ErrNr] = -900000.0; XCH4_val[i+ErrNr] = -900000.0;
        if (float(fid['XCO'][i+ErrNr])  == -900000.0): XCO_err[i+ErrNr]  = -900000.0; XCO_val[i+ErrNr]  = -900000.0;

    # uncertainty for the last entry (for each species)

      for i in range(ErrNr):

        XH2O_val[i] = float(fid['XH2O'][i])
        XCO2_val[i] = float(fid['XCO2'][i]) # * float(fid['XCO2_cal'][...])
        XCH4_val[i] = float(fid['XCH4'][i]) # * float(fid['XCH4_cal'][...])
        XCO_val[i]  = float(fid['XCO'][i]) * 1000.0
        ...
        if (float(fid['XH2O'][i]) == -900000.0): XH2O_err[i] = -900000.0; XH2O_val[i] = -900000.0;
        if (float(fid['XCO2'][i]) == -900000.0): XCO2_err[i] = -900000.0; XCO2_val[i] = -900000.0;
        if (float(fid['XCH4'][i]) == -900000.0): XCH4_err[i] = -900000.0; XCH4_val[i] = -900000.0;
        if (float(fid['XCO'][i])  == -900000.0): XCO_err[i]  = -900000.0; XCO_val[i]  = -900000.0;

      for i in range(ErrNr):

        XH2O_val[fid['DateTime'].shape[0]-i-1] = float(fid['XH2O'][fid['DateTime'].shape[0]-i-1])
        XCO2_val[fid['DateTime'].shape[0]-i-1] = float(fid['XCO2'][fid['DateTime'].shape[0]-i-1]) # * float(fid['XCO2_cal'][...])
        XCH4_val[fid['DateTime'].shape[0]-i-1] = float(fid['XCH4'][fid['DateTime'].shape[0]-i-1]) # * float(fid['XCH4_cal'][...])
        XCO_val[fid['DateTime'].shape[0]-i-1]  = float(fid['XCO'][fid['DateTime'].shape[0]-i-1]) * 1000.0
        ...
        if (float(fid['XH2O'][fid['DateTime'].shape[0]-i-1]) == -900000.0): XH2O_err[fid['DateTime'].shape[0]-i-1] = -900000.0; XH2O_val[fid['DateTime'].shape[0]-i-1] = -900000.0;
        if (float(fid['XCO2'][fid['DateTime'].shape[0]-i-1]) == -900000.0): XCO2_err[fid['DateTime'].shape[0]-i-1] = -900000.0; XCO2_val[fid['DateTime'].shape[0]-i-1] = -900000.0;
        if (float(fid['XCH4'][fid['DateTime'].shape[0]-i-1]) == -900000.0): XCH4_err[fid['DateTime'].shape[0]-i-1] = -900000.0; XCH4_val[fid['DateTime'].shape[0]-i-1] = -900000.0;
        if (float(fid['XCO'][fid['DateTime'].shape[0]-i-1])  == -900000.0): XCO_err[fid['DateTime'].shape[0]-i-1]  = -900000.0; XCO_val[fid['DateTime'].shape[0]-i-1]  = -900000.0;

      GEOMS_store(dict(name='H2O.COLUMN.MIXING.RATIO.VOLUME.DRY_ABSORPTION.SOLAR',dtype='REAL',unit='ppmv',desc='Column average dry air mole fraction',dep='DATETIME'),XH2O_val,si='0.0;1.0E-6;1')
      GEOMS_store(dict(name='CO2.COLUMN.MIXING.RATIO.VOLUME.DRY_ABSORPTION.SOLAR',dtype='REAL',unit='ppmv',desc='Column average dry air mole fraction',dep='DATETIME'),XCO2_val,si='0.0;1.0E-6;1')
      GEOMS_store(dict(name='CH4.COLUMN.MIXING.RATIO.VOLUME.DRY_ABSORPTION.SOLAR',dtype='REAL',unit='ppmv',desc='Column average dry air mole fraction',dep='DATETIME'),XCH4_val,si='0.0;1.0E-6;1')
      GEOMS_store(dict(name='CO.COLUMN.MIXING.RATIO.VOLUME.DRY_ABSORPTION.SOLAR', dtype='REAL',unit='ppbv',desc='Column average dry air mole fraction',dep='DATETIME'),XCO_val,si='0.0;1.0E-9;1')
...
# ...
    ...
    # H_len = len(fid['H_prior']) - 1
      ...
      GEOMS_store(dict(name='DRY.AIR.COLUMN.PARTIAL_INDEPENDENT',dtype='REAL',unit='Zmolec cm-2',desc='Vertical profile of partial columns of air number densities (for conversion between VMR and partial column profile)',dep='DATETIME;ALTITUDE'),X/1.0E25,si='0.0;1.66054E1;mol m-2')
...
# file attributes
      ...
      gfid.attrs['DATA_DESCRIPTION'] = np.array('%s measurements from %s '%(iname,isite))
      gfid.attrs['DATA_VARIABLES'] = np.array(';'.join(variableswritten))
      gfid.attrs['DATA_TEMPLATE'] = np.array('GEOMS-TE-FTIR-COCCON-001.csv')
    # gfid.attrs['DATA_TEMPLATE'] = np.array(template)
    # gfid.attrs['DATA_TEMPLATE'] = np.array(os.path.basename(os.path.splitext(template_path)[0]))
      gfid.attrs['DATA_QUALITY'] = np.array(fid.attrs['DATA_QUALITY'])
      gfid.attrs['DATA_MODIFICATIONS'] = np.array('Calibration factors applied: %s for XCO2 and %s for XCH4'%(str(fid['XCO2_cal'][...]),str(fid['XCH4_cal'][...])))
    ...
    # gfid.attrs['FILE_META_VERSION'] = np.array('%s;CUSTOM'%table[-10:-4])
    # gfid.attrs['FILE_META_VERSION'] = np.array(os.path.splitext(table_path)[0]).split('_')[1]))
