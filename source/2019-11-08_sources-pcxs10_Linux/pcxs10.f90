...
real,dimension(:),allocatable :: sumtau_zenith,tau_cumul
...
! call tofile_solspec_inp('solspec_inp.dat',firstnue_solspec_inp,lastnue_solspec_inp,ngrid_solspec_inp,solspec_inp)
...
call tofile_solspec('solspec.dat',solspec)
...
call tofile_lblinfos('lblpointer.dat')
...
allocate (sumtau_zenith(wvskal%ngrid),tau_cumul(wvskal%ngrid))
...
do i = 1,n_tau
    ...
    call tofile_sumtau('sumtau',i,1,sumtau)

    ! for each species: calculate AVKs (approximated)
    print *,'Calculating AVKs ...'
    tau_cumul(:) = tau_cumul(:) + sumtau(:,1)
    call make_AVK(i,tau_lev,tau_cumul)
    ...
    call tofile_tau('dtau_dT',i,dtau_dT)
    ...
    call tofile_tau('dtau_dp',i,dtau_dp)
...
deallocate (sumtau_zenith,tau_cumul)
deallocate (solspec)
print *,'...deallocated!'
print *,''
...
!====================================================================
subroutine make_AVK(igas,tau_lev,tau_cumul)
...
real,dimension(wvskal%ngrid),intent(in) :: tau_cumul
...
do i = 1,maxavk
    ...
    ! calculate trm (plane parallel approx)
    trm(1-ilsradius:mw(igas)%nfine+ilsradius) = exp(-tau_cumul(istart:istop) / cos(obs%sza_gnd_rad_avk(i)))
...
do j = ilev_low,ilev_up
    ...
    ! Abklingende Linienflügel berechnen
...
print *,'solar spectrum file:       ',pTdatei
...
use globvar10, only : pathstr,iunit_fwd
